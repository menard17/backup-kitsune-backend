...
class MockPatientClient:
    ...
    def get_resource(self, id: str, resource_type: str) -> Patient:
        resource = construct_fhir_element(resource_type, json.dumps(self.data))
        return Patient(**resource.dict())
...
def test_get_patient_email_normal():
    ...
    mock.get_resource = MockPatientClient(email=expected_email).get_resource
    patient_service = PatientService(mock)

    # When
    _, actual_email = patient_service.get_patient_email(uuid.uuid1())
...
def test_get_patient_email_not_exist():
    # Given
    mock = Mock(ResourceClient)
    mock.get_resource = MockPatientClient().get_resource
    patient_service = PatientService(mock)
...
def test_get_patient_name_normal():
    ...
    # When
    _, actual_name = patient_service.get_patient_name(uuid.uuid1())
...
def test_check_link_return_false_when_not_link_for_patient():
    ...
    assert not ok
    assert err_resp and err_resp.status_code == 400
    assert err_resp and err_resp.data == b"not link for patient"
...
def test_get_patient_multiple_emails():
    ...
    mock = Mock(ResourceClient)
    mock.get_resource = MockPatientClient(
        email=new_email, second_email=old_email
    ).get_resource
    patient_service = PatientService(mock)
...
def test_get_patient_payment_details():
    ...
    err, (
        ...
        expected_payment_id,
    ) = patient_service.get_patient_payment_details(uuid.uuid1())
...
def test_get_patient_payment_error():
    ...
    # When
    err, outputs = patient_service.get_patient_payment_details(uuid.uuid1())
...
def test_get_kana_empty():
    ...
    resource = construct_fhir_element("Patient", mock_resource_client.data)
